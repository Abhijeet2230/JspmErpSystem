...
public class ExcelImportServiceImpl implements ExcelImportService {
    ...
    public int importStudents(MultipartFile file) {
        return importStudentsInternal(file, 0, false); // full import, headerRowNumber ignored
    }

    @Override
    @Transactional
    public int importStudentsBasic(MultipartFile file, int headerRowNumber) {
        return importStudentsInternal(file, headerRowNumber, true); // basic import
    }

    // ---------- Internal method to handle both imports ----------
    private int importStudentsInternal(MultipartFile file, int headerRowNumber, boolean isBasic) {
        try {
            List<StudentDTO> studentDTOs;
            if (isBasic) {
                studentDTOs = ExcelHelper.excelToBasicStudentDTOs(file.getInputStream(), headerRowNumber);
            } else {
                studentDTOs = ExcelHelper.excelToStudentDTOs(file.getInputStream());
            }
            ...
            Role studentRole = roleRepository.findByRoleName(AppRole.ROLE_STUDENT)
                    .orElseThrow(() -> new ExcelImportException("ROLE_STUDENT not found"));

            Course defaultCourse = null;
            if (isBasic) {
                defaultCourse = courseRepository.findById(1L)
                        .orElseThrow(() -> new ExcelImportException("Course with ID=1 not found"));
            }
            ...
            for (StudentDTO dto : studentDTOs) {
                ...
                // ---------- Create User ----------
                User user = new User();
                user.setUserName(isBasic ? String.valueOf(dto.getRollNo()) : dto.getApplicationId());
                ...
                // ---------- Create Student ----------
                Student student = isBasic ? new Student() : studentMapper.toEntity(dto);
                if (isBasic) {
                ...
                // ---------- Gender ----------
                if (!isBasic && dto.getGender() != null && !dto.getGender().isBlank()) {
                    try {
                        student.setGender(Gender.valueOf(dto.getGender().toUpperCase().replace(" ", "_")));
                    } catch (IllegalArgumentException ignored) {
                        log.warn("Invalid gender for student: {}", dto.getGender());
                    }
                }

                // ---------- Parent ----------
                if (!isBasic && dto.getParent() != null) {
                ...
                // ---------- Address ----------
                if (!isBasic && dto.getAddress() != null) {
                ...
                // ---------- Entrance Exams ----------
                if (!isBasic && dto.getEntranceExams() != null && !dto.getEntranceExams().isEmpty()) {
                ...
                // ---------- Admissions ----------
                List<AdmissionDTO> admissionDTOs = !isBasic && dto.getAdmissions() != null ? dto.getAdmissions() : new ArrayList<>();
            ...
            throw new ExcelImportException("Failed to import Excel file: " + e.getMessage(), e);
        }
    }
}
